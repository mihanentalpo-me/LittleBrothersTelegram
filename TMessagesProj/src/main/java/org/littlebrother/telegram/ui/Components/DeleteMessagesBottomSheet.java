...
import static org.littlebrother.telegram.ui.Components.UniversalAdapter.VIEW_TYPE_EXPANDABLE_SWITCH;
import static org.littlebrother.telegram.ui.Components.UniversalAdapter.VIEW_TYPE_ROUND_CHECKBOX;
import static org.littlebrother.telegram.ui.Components.UniversalAdapter.VIEW_TYPE_SHADOW_COLLAPSE_BUTTON;
import static org.littlebrother.telegram.ui.Components.UniversalAdapter.VIEW_TYPE_SWITCH;
import static org.littlebrother.telegram.ui.Components.UniversalAdapter.VIEW_TYPE_USER_GROUP_CHECKBOX;
import static org.littlebrother.telegram.ui.Components.UniversalAdapter.VIEW_TYPE_USER_CHECKBOX;
...
import org.littlebrother.telegram.ui.Cells.CollapseTextCell;
import org.littlebrother.telegram.ui.Components.Premium.boosts.cells.selector.SelectorBtnCell;
        ...
        banOrRestrict.forEachSelected((participant, i) -> {
            if (restrict) {
                ...
                if (participant instanceof TLRPC.User) {
                    MessagesController.getInstance(currentAccount).setParticipantBannedRole(inChat.id, (TLRPC.User) participant, null, rights, false, getBaseFragment());
                } else if (participant instanceof TLRPC.Chat) {
                    MessagesController.getInstance(currentAccount).setParticipantBannedRole(inChat.id, null, (TLRPC.Chat) participant, rights, false, getBaseFragment());
                }
            } else {
                if (participant instanceof TLRPC.User) {
                    MessagesController.getInstance(currentAccount).deleteParticipantFromChat(inChat.id, (TLRPC.User) participant, null, false, false);
                } else if (participant instanceof TLRPC.Chat) {
                    MessagesController.getInstance(currentAccount).deleteParticipantFromChat(inChat.id, null, (TLRPC.Chat) participant, false, false);
                }
            }
        });
